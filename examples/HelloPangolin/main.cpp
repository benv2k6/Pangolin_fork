...
class RgoHandler3d : public pangolin::Handler3D {
   public:
    ...
    void Keyboard(pangolin::View& view, unsigned char key, int x, int y,
        ...
        const Eigen::Matrix3d R_env_cam = mvmat.block<3, 3>(0, 0).transpose();
        switch (key) {
            case pangolin::PANGO_SPECIAL + pangolin::PANGO_KEY_LEFT:
                transform.block<3, 1>(0, 3) =
                    R_env_cam * unit_vec_left_ * move_factor_;
                break;
            ...
            case pangolin::PANGO_SPECIAL + pangolin::PANGO_KEY_UP:
                transform.block<3, 1>(0, 3) =
                    R_env_cam * unit_vec_forward_ * move_factor_;
                break;
            ...
            case 'a':
                transform.block<3, 3>(0, 0) =
                    Eigen::Quaternion<double>(R_env_cam).matrix();
                break;
            case 'd':
                transform.block<3, 3>(0, 0) =
            ...
            case 'q':
                transform.block<3, 3>(0, 0) =
                    Eigen::AngleAxis<double>(
                        ...
                        (R_env_cam * unit_vec_forward_).normalized())
                        .matrix();
                break;
            case 'e':
                transform.block<3, 3>(0, 0) =
            ...
            case 'w':
                transform.block<3, 3>(0, 0) =
                    Eigen::AngleAxis<double>(
                        degreesToRadians(-rotation_factor_deg_),
                        (R_env_cam * unit_vec_left_).normalized())
                        .matrix();
                break;
            case 's':
                transform.block<3, 3>(0, 0) =
        ...
        cam_state->SetModelViewMatrix(
            static_cast<Eigen::Matrix4d>(mvmat * transform));
    }
